...
gm = GaussianMixture(n_components=len(data_categ.columns), random_state=0).fit(data_categ)

predi=gm.predict(data_categ)
data_categ2["predicted_valu"]=predi
data_categ2.to_csv(path_to_result+r"\results6.csv",decimal = ".",sep=";")
data_categ2.to_excel(path_to_result+r"\results6.xlsx")
